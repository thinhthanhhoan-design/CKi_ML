...
def main(
    ...
    df_clean.to_csv("tables/deeplearwing_day1_clean.csv", index=False)
    n_unique_airfoils = (
    ...
    print(
        ...
        f"output={os.path.abspath('tables/deeplearwing_day1_clean.csv')}"
    )
    ...
    try:
        ...
        for ax, (col, title) in zip(axes, [("cl", "Cl"), ("cd", "Cd"), ("cm", "Cm")]):
            means  = [df_all.loc[df_all["flow_regime"] == r, col].mean() for r in regimes_ord]
            stds   = [df_all.loc[df_all["flow_regime"] == r, col].std()  for r in regimes_ord]
    ...
    try:
        ...
        regimes_ord = ["linear", "transitional", "pre_stall", "near_stall", "post_stall"]
        box_data = [
            df_all.loc[df_all["flow_regime"] == r, "cd"].dropna().values
