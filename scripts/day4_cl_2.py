...
logger = logging.getLogger("CL_Reliability_V9_HGB")

OUTPUT_ENCODING = "utf-8-sig"
CL_HGB_MAX_ITER = 300
CL_HGB_MAX_DEPTH = 8
CL_HGB_MAX_LEAF_NODES = 63
CL_HGB_L2_REGULARIZATION = 0.05
...
class CLReliabilityConfig:
    ...
    output_dir: str = "outputs/day4_cl_2_hgb"
    n_splits: int = 10
    random_state: int = 42
    ...
    extreme_weight_add: float = 0.35
    near_clmax_weight_add: float = 0.35
    max_sample_weight: float = 1.80
...
def get_model_pipeline(name: str, rs: int, n_jobs: int = -1) -> Pipeline:
    """Factory HGB-only cho pipeline CL sạch."""
    if name != "hgb":
        raise ValueError(f"Unknown architecture core request: {name}")
    return Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("model", HistGradientBoostingRegressor(
            max_iter=int(CL_HGB_MAX_ITER),
            max_depth=CL_HGB_MAX_DEPTH,
            max_leaf_nodes=CL_HGB_MAX_LEAF_NODES,
            learning_rate=0.05,
            l2_regularization=CL_HGB_L2_REGULARIZATION,
            random_state=rs,
        ))
    ])
...
def build_cl_sample_weights(df: pd.DataFrame, y: np.ndarray, cfg: CLReliabilityConfig) -> np.ndarray:
    """Mildly prioritize CL extremes and measured near-CLmax samples without dominating the loss."""
    weights = np.ones(len(y), dtype=np.float32)
    abs_y = np.abs(y.astype(np.float64, copy=False))
    q75, q98 = np.quantile(abs_y, [0.75, 0.98])
    extreme_scale = np.clip((abs_y - q75) / max(float(q98 - q75), 1e-6), 0.0, 1.0)
    weights += float(cfg.extreme_weight_add) * extreme_scale.astype(np.float32)

    keys = ["geom_hash", "log10_re"] if "log10_re" in df.columns else ["geom_hash"]
    curve_max = df.groupby(keys, sort=False)["cl"].transform("max").to_numpy(dtype=np.float32)
    angles = df["angle"].to_numpy(dtype=np.float32)
    near_clmax = (y >= curve_max - 0.12) & (angles >= 4.0)
    weights[near_clmax] += float(cfg.near_clmax_weight_add)

    weights = np.clip(weights, 1.0, float(cfg.max_sample_weight))
...
def section_base_cl_model(df: pd.DataFrame, features: List[str], cfg: CLReliabilityConfig, paths: Dict[str, Path]) -> Tuple[Dict, np.ndarray, np.ndarray]:
    ...
    sample_weights = build_cl_sample_weights(df, y, cfg)
    pd.DataFrame(
        [{
            "weight_mean": float(np.mean(sample_weights)),
            "weight_min": float(np.min(sample_weights)),
            "weight_max": float(np.max(sample_weights)),
            "weighted_rows": int(np.sum(sample_weights > 1.0)),
        }]
    ).to_csv(paths["tables"] / "cl_sample_weight_summary.csv", index=False, encoding=OUTPUT_ENCODING)
    ...
    for cand_i, name in enumerate(candidates, start=1):
        ...
        for fold_i, (tr_idx, va_idx) in enumerate(gkf.split(X, y, groups=groups), start=1):
            ...
            m.fit(X[tr_idx], y[tr_idx], model__sample_weight=sample_weights[tr_idx])
    ...
    final_model.fit(X, y, model__sample_weight=sample_weights)
    model_store = {best_name: (final_model, [])}
    gc.collect()

    return {
        "best_name": best_name, "model_store": model_store, "oof_store": oof_store,
        "X": X, "y": y, "groups": groups, "sample_weights": sample_weights,
        "feat_cols": features, "n_splits": n_splits
...
def section_clmax_surrogate(df: pd.DataFrame, geom_features: List[str], cfg: CLReliabilityConfig, paths: Dict[str, Path]) -> Tuple[Dict, object]:
    logger.info("[S5] Training Dedicated CLmax Surrogate via HGB-only GroupKFold...")
...
def section_stall_surrogate(clmax_ctx: Dict, cfg: CLReliabilityConfig, paths: Dict[str, Path]) -> object:
    logger.info("[S6] Training Dedicated Stall AoA Surrogate via HGB-only GroupKFold...")
...
def section_uncertainty_and_coverage(base_ctx: Dict, cfg: CLReliabilityConfig, paths: Dict[str, Path]) -> Dict:
    ...
    gc.collect()
    return {
        "uq_scores_train": uq_scores_train,
        "residuals_train": residuals_all,
        "calibration_mask": cal_mask,
        "evaluation_mask": ~cal_mask,
        "quantiles": conformal_meta,
    }
...
def section_physical_consistency_suite(df: pd.DataFrame, oof_preds: np.ndarray) -> int:
    ...
    for _, grp in tmp.groupby(group_cols):
        ...
        grp_s = grp.groupby("angle", as_index=False)["pred"].median().sort_values("angle")
        if len(grp_s) < 4:
            continue
        aoa = grp_s["angle"].to_numpy(float)
        cl_p = grp_s["pred"].to_numpy(float)
        linear_mask = (aoa >= -4) & (aoa <= 6)
        if linear_mask.sum() >= 3 and np.ptp(aoa[linear_mask]) >= 4.0:
            linear_slope = float(np.polyfit(aoa[linear_mask], cl_p[linear_mask], 1)[0])
            if linear_slope < 0.02:
                violations += 1
                continue
...
def section_bootstrap_ensemble(base_ctx: Dict, conformal_ctx: Dict, cfg: CLReliabilityConfig, paths: Dict[str, Path]) -> Dict:
    logger.info("[V9-E] Training bootstrap ensemble with adaptive split-conformal intervals...")
    X, y, best_name = base_ctx["X"], base_ctx["y"], base_ctx["best_name"]
    sample_weights = base_ctx["sample_weights"]
    n = len(y)
    ...
    def _fit_bootstrap_member(i: int) -> Tuple[Pipeline, np.ndarray]:
        ...
        m.fit(X[idx], y[idx], model__sample_weight=sample_weights[idx])
    ...
    results = None
    gc.collect()
    raw_p10 = np.percentile(pred_arr, 10, axis=0)
    raw_p50 = np.percentile(pred_arr, 50, axis=0)
    raw_p90 = np.percentile(pred_arr, 90, axis=0)
    raw_half_width = np.maximum((raw_p90 - raw_p10) / 2.0, 1e-5)
    median_half_width = max(float(np.median(raw_half_width)), 1e-5)
    adaptive_scale = np.clip(raw_half_width / median_half_width, 0.50, 2.50)

    oof_center = base_ctx["oof_store"][best_name]
    oof_error = np.abs(y - oof_center)
    cal_mask = conformal_ctx["calibration_mask"]
    normalized_scores = oof_error[cal_mask] / adaptive_scale[cal_mask]
    q90 = float(np.quantile(normalized_scores, 0.90))
    q95 = float(np.quantile(normalized_scores, 0.95))
    radius90 = q90 * adaptive_scale
    radius95 = q95 * adaptive_scale

    uq_df = pd.DataFrame({
        "cl_bootstrap_p10": raw_p10,
        "cl_p50": raw_p50,
        "cl_bootstrap_p90": raw_p90,
        "cl_p90_lo": raw_p50 - radius90,
        "cl_p90_hi": raw_p50 + radius90,
        "cl_p95_lo": raw_p50 - radius95,
        "cl_p95_hi": raw_p50 + radius95,
    })
    # Backward-compatible aliases now represent a calibrated 90% interval.
    uq_df["cl_p10"] = uq_df["cl_p90_lo"]
    uq_df["cl_p90"] = uq_df["cl_p90_hi"]
    uq_df["uncertainty_width"] = 2.0 * radius90
    ...
    interval_meta = {
        "method": "adaptive_split_conformal_on_oof_residuals",
        "q90_normalized": q90,
        "q95_normalized": q95,
        "median_bootstrap_half_width": median_half_width,
        "scale_clip": [0.50, 2.50],
    }
    joblib.dump(interval_meta, paths["models"] / "bootstrap_interval_calibrator.joblib")
    return {
        "models": models,
        "predictions": uq_df,
        "pred_matrix": pred_arr,
        "radius90": radius90,
        "radius95": radius95,
...
def section_uncertainty_quality(y_true: np.ndarray, base_ctx: Dict, conformal_ctx: Dict, bootstrap_ctx: Dict, paths: Dict[str, Path]) -> Dict:
    logger.info("[V9-G] Validating uncertainty quality gate...")
    uq = bootstrap_ctx["predictions"]
    oof_center = base_ctx["oof_store"][base_ctx["best_name"]]
    err = np.abs(y_true - oof_center)
    ...
    eval_mask = conformal_ctx["evaluation_mask"]
    radius90 = bootstrap_ctx["radius90"]
    radius95 = bootstrap_ctx["radius95"]
    cov90 = float(np.mean(err[eval_mask] <= radius90[eval_mask]))
    cov95 = float(np.mean(err[eval_mask] <= radius95[eval_mask]))
    try:
        slope = float(np.polyfit(width, err, 1)[0]) if np.std(width) > 0 else 0.0
    except Exception:
        slope = 0.0
    # A slightly conservative 90% interval is preferable to under-coverage.
    # Keep the gate narrow, but allow up to 93% to avoid rejecting sound UQ
    # because of finite-sample conformal quantile rounding.
    reliable = bool((pear > 0.25) and (spear > 0.25) and (0.88 <= cov90 <= 0.93) and (0.93 <= cov95 <= 0.97))
    metrics = pd.DataFrame([{
        "interval_method": "adaptive_split_conformal",
        "pearson_uq_error": pear,
        "spearman_uq_error": spear,
        "coverage90": cov90,
        "coverage90_gap": cov90 - 0.90,
        "coverage95": cov95,
        "coverage95_gap": cov95 - 0.95,
...
def section_active_learning(clean_df: pd.DataFrame, bootstrap_ctx: Dict, trust_df: pd.DataFrame, paths: Dict[str, Path]) -> pd.DataFrame:
    ...
    group_cols = ["geom_hash", "log10_re"] if "log10_re" in clean_df.columns else ["geom_hash"]
    for _, idx in clean_df.groupby(group_cols, sort=False).groups.items():
        ii = list(idx)
        curve = pd.DataFrame({
            "row_index": ii,
            "angle": clean_df.iloc[ii]["angle"].to_numpy(float),
            "pred": uq.iloc[ii]["cl_p50"].to_numpy(float),
        })
        unique_curve = curve.groupby("angle", as_index=False)["pred"].median().sort_values("angle")
        if len(unique_curve) < 3 or np.ptp(unique_curve["angle"].to_numpy(float)) <= 0:
            continue
        aoa_unique = unique_curve["angle"].to_numpy(float)
        pred_unique = unique_curve["pred"].to_numpy(float)
        grad = np.gradient(pred_unique, aoa_unique)
        sens_unique = np.abs(np.gradient(grad, aoa_unique))
        sensitivity_by_angle = dict(zip(aoa_unique, sens_unique))
        mapped = curve["angle"].map(sensitivity_by_angle).fillna(0.0).to_numpy(float)
        out.iloc[ii, out.columns.get_loc("stall_sensitivity")] = mapped
...
def section_stall_physics_audit(clean_df: pd.DataFrame, pred: np.ndarray, paths: Dict[str, Path]) -> pd.DataFrame:
    ...
    for key, grp in tmp.groupby(group_cols):
        # Duplicate AoA rows can create zero-spacing derivatives; aggregate them first.
        grp = grp.groupby("angle", as_index=False)[["cl", "pred"]].median().sort_values("angle")
        aoa = grp["angle"].to_numpy(float)
        cl_true = grp["cl"].to_numpy(float)
        clp = grp["pred"].to_numpy(float)
        linear = (aoa >= -4) & (aoa <= 6)
        neg_slope = False
        linear_slope = np.nan
        if linear.sum() >= 3 and np.ptp(aoa[linear]) >= 4.0:
            linear_slope = float(np.polyfit(aoa[linear], clp[linear], 1)[0])
            neg_slope = bool(linear_slope < 0.02)
        clmax_i = int(np.argmax(clp))
        true_clmax_i = int(np.argmax(cl_true))
        pred_peak_at_end = bool(aoa[clmax_i] >= np.nanmax(aoa) - 1e-8)
        true_peak_at_end = bool(aoa[true_clmax_i] >= np.nanmax(aoa) - 1e-8)
        # Do not call a boundary peak a model failure when the measured curve also ends at its peak.
        late_peak = bool(pred_peak_at_end and not true_peak_at_end and np.nanmax(aoa) >= 12.0)
        rows.append({
            "group": str(key),
            "linear_slope": linear_slope,
            "negative_linear_slope": neg_slope,
            "clmax_at_last_aoa": late_peak,
            "true_clmax_at_last_aoa": true_peak_at_end,
            "violation": neg_slope or late_peak,
        })
    out = pd.DataFrame(rows)
...
def section_export_day5_interfaces_v9(feat_cols: List[str], base_ctx: Dict, clmax_ctx: Dict, clean_df: pd.DataFrame, paths: Dict[str, Path], uq_quality: Dict) -> None:
    ...
    meta.update({
        ...
        "uncertainty_quality_gate": "pearson>0.25, spearman>0.25, coverage90=0.88..0.93, coverage95=0.93..0.97",
        "artifacts_mapping": {**meta.get("artifacts_mapping", {}),
            "bootstrap_cl_ensemble": "models/bootstrap_cl_ensemble.joblib",
            "bootstrap_interval_calibrator": "models/bootstrap_interval_calibrator.joblib",
...
def main() -> None:
    ...
    status_path = paths["base"] / "pipeline_status.json"
    ...
    bootstrap_ctx = section_bootstrap_ensemble(base_ctx, uq_ctx, cfg, paths)
    # Regime calibration must use geometry-held-out OOF predictions, not in-sample bootstrap centers.
    regime_conf = section_regime_conformal(clean_df, y_true, oof_preds, cfg, paths)
    uq_quality = section_uncertainty_quality(y_true, base_ctx, uq_ctx, bootstrap_ctx, paths)
    ...
    required_outputs = [
        paths["models"] / "cl_model.joblib",
        paths["models"] / "clmax_model.joblib",
        paths["models"] / "stall_model.joblib",
        paths["models"] / "bootstrap_cl_ensemble.joblib",
        paths["models"] / "bootstrap_interval_calibrator.joblib",
        paths["figures"] / "cl_prediction_vs_truth.png",
        paths["figures"] / "cl_error_by_reynolds_bin.png",
        paths["artifacts"] / "day5_interface.json",
        paths["reports"] / "day4_cl_v9_publication_report.md",
    ]
    missing_outputs = [str(path) for path in required_outputs if not path.exists()]
    status_path.write_text(
        json.dumps(
            {
                "status": "COMPLETE" if not missing_outputs else "INCOMPLETE",
