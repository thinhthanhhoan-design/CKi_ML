...
class AirfoilPreprocessingPipeline:
    ...
    def assemble_final_dataset(self, raw_csv_path: str, chunksize: int = 100_000) -> None:
        ...
        # Compress final CSV to .csv.gz and delete raw .csv
        import gzip
        import shutil
        print(f"-> [Day2] Compressing final dataset {output_csv} to {output_csv}.gz...")
        with open(output_csv, 'rb') as f_in:
            with gzip.open(output_csv + ".gz", 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        os.remove(output_csv)
        
        print(f"=== [DAY2] DONE: {output_csv}.gz ===")
...
if __name__ == "__main__":
    csv_input_path = "tables/deeplearwing_day1_clean.csv.gz"
